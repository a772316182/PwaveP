...
def farthest_point_sample(points, num_samples):
    """
    使用 NumPy 实现最远点采样 (Farthest Point Sampling, FPS)。
    (此函数保持不变)
    """
    ...
    for i in range(1, num_samples):
        ...
        dist_sq = np.sum((points - last_sampled_point) ** 2, axis=1)
        min_distances = np.minimum(min_distances, dist_sq)
        next_point_idx = np.argmax(min_distances)
        sampled_indices[i] = next_point_idx
