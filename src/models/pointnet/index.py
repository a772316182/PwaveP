...
class PointNet(nn.Module):
    ...
    def forward(self, pc, need_feature=False, **kwags):
        if pc.shape[-1] != 3:
            pc = pc.permute(0, 2, 1)

        pc = pc.transpose(2, 1).float()
        logit, _, trans_feat = self.model.forward(pc, need_feature)
        out = {"logit": logit, "trans_feat": trans_feat, "feature": trans_feat}
        return out
