...
from .backbone import build_eidos_attack


def start_eidos_attack(
    model, device, loader_train, loader_test, num_batches_to_attack, args
):
    logger.info(f"Start eidos attack on {args.dataset} dataset, with {args.model} model")
    ...
    for batch_idx, data_item in enumerate(tqdm(loader_test)):
        ...
        eidos = build_eidos_attack(model, num_classes)

        adv_pc, target, _ = eidos.forward(
