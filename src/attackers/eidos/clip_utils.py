...
class ClipPointsL2(nn.Module):
    ...
    def forward(self, pc, ori_pc):
        ...
        with torch.no_grad():
            ...
            scale_factor = torch.clamp(scale_factor, max=1.0)  # [B]
...
class ProjectInnerPoints(nn.Module):
    ...
    def forward(self, pc, ori_pc, normal=None):
        ...
        with torch.no_grad():
            ...
            inner_mask = inner_diff_normal < 0.0  # [B, K]
            ...
            diff_proj = diff * vref / (vref_norm[:, None, :] + 1e-9)  # [B, 3, K]

            # some diff is completely opposite to normal
            # just set them to (0, 0, 0)
            opposite_mask = inner_mask & (vng_norm < 1e-6)
            opposite_mask = opposite_mask.unsqueeze(1).expand_as(diff_proj)
            diff_proj[opposite_mask] = 0.0

            # set inner points with projected perturbation
            inner_mask = inner_mask.unsqueeze(1).expand_as(diff)
