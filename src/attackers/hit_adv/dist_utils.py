...
class ChamferDist(nn.Module):
    ...
    def forward(self, adv_pc, ori_pc,
        ...
        if weights is None:
            weights = torch.ones((B,))
        loss1, loss2 = ChamferDistance().forward(adv_pc, ori_pc)  # [B], adv2ori, ori2adv
        if self.method == 'adv2ori':
            ...
            loss = (loss1 + loss2) / 2.
        weights = weights.to(ori_pc.device).float()
        loss = loss * weights
        if batch_avg:
            return loss.mean()
        return loss
...
class HausdorffDist(nn.Module):
    ...
    def forward(self, adv_pc, ori_pc,
        ...
        if weights is None:
            weights = torch.ones((B,))
        loss1, loss2 = HausdorffDistance().forward(adv_pc, ori_pc)  # [B], adv2ori, ori2adv
        if self.method == 'adv2ori':
            loss = loss1
        elif self.method == 'ori2adv':
            loss = loss2
