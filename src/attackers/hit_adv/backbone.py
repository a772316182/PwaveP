...
class UntargetedLogitsAdvLoss(torch.nn.Module):

    def __init__(self, device, kappa=0.0):
    ...
    def forward(self, logits, targets):
        ...
        one_hot_targets = (
            torch.zeros(B, K).to(self.device).scatter(1, targets, 1).float()
        )  # to one-hot
        real_logits = torch.sum(one_hot_targets * logits, dim=1)
        other_logits = torch.max(
            (1.0 - one_hot_targets) * logits - one_hot_targets * 10000.0, dim=1
        )[0]
        loss = torch.clamp(real_logits - other_logits + self.kappa, min=0.0)
        return loss.mean()
...
class HiT_ADV:
    ...
    def __init__(
        ...
        self.model.eval()
        self.adv_func = UntargetedLogitsAdvLoss(device)
    ...
    def attack(self, data, target):
        ...
        ori_data = data[:, :, :3].float().to(self.device).clone().detach()
        ori_normal = (
            data[:, :, 3:].float().to(self.device).clone().detach()
        )  # [B, N, 3]
        ...
        target = target.long().to(self.device).detach()
        label_val = target.detach().cpu().numpy()  # [B]

        ori_kappa_std = self._get_kappa_std_ori(
            ori_data, ori_normal, k=self.curv_loss_knn
        )  # [b, n]
        ...
        with torch.no_grad():
            ...
            saliency = (
                -1.0
                * (r ** (self.alpha))
                * torch.sum((ori_data - center[:, :, None]) * grad, dim=1)
            )  # [B, K]
            normalized_saliency = (saliency - torch.min(saliency)) / (
                torch.max(saliency) - torch.min(saliency) + 1e-7
            )  # [B, K]
            normalized_std = (ori_kappa_std - torch.min(ori_kappa_std)) / (
                torch.max(ori_kappa_std) - torch.min(ori_kappa_std) + 1e-7
            )  # [B, K]
            score = 0.001 * normalized_saliency + normalized_std  # [B, K]
            # score = normalized_std  # [B, K]

            far_points_idx = self.farthest_point_sample(
                ori_data.transpose(1, 2).contiguous(), self.total_central_num
            )  # [B, central_points]
            far_points = self.index_points(
                ori_data.transpose(1, 2).contiguous(), far_points_idx
            )  # [B, central_points, 3]
            far_knn = knn_points(
                far_points, ori_data.permute(0, 2, 1), K=self.curv_loss_knn + 1
            )  # [dists:[b,central_points,k+1], idx:[b,central_points,k+1]]
            far_knn_points = (
                knn_gather(ori_data.permute(0, 2, 1), far_knn.idx)
                .permute(0, 3, 1, 2)
                .contiguous()
            )
            ...
            total_central_points = self.index_points(
                far_knn_points.permute(0, 2, 3, 1)
                .contiguous()
                .view(-1, self.curv_loss_knn + 1, 3),
                total_central_points_idx.view(-1, 1),
            )
            total_central_points = (
                total_central_points.view(B, -1, 3).transpose(1, 2).contiguous()
            )
            total_central_points_score = self.index_points(
                far_knn_score.view(-1, self.curv_loss_knn + 1, 1),
                total_central_points_idx.view(-1, 1),
            ).view(B, -1)

            central_points_score, tmp_idx = torch.topk(
                total_central_points_score, k=self.central_num
            )
            central_points = (
                self.index_points(
                    total_central_points.transpose(1, 2).contiguous(), tmp_idx
                )
                .transpose(1, 2)
                .contiguous()
            )
        ...
        total_central_kappa_std = self.index_points(
            far_kappa_std.view(-1, self.curv_loss_knn + 1, 1),
            total_central_points_idx.view(-1, 1),
        ).view(B, -1, 1)
        ...
        for binary_step in range(self.binary_step):
            ...
            perturb_mat = (torch.rand(B, self.central_num, 3) * init_trans_scale).to(
                self.device
            )

            # init delta for Gauss Kernel  [B, central_num]
            gauss_delta = torch.ones((B, self.central_num)).to(
                self.device
            ) * self.min_sigm + torch.rand((B, self.central_num)).to(self.device) * (
                self.max_sigm - self.min_sigm
            )
            ...
            for iteration in range(self.num_iter):
                ...
                tmp_adv_data = torch.zeros_like(adv_data).to(self.device)
                tmp_deno = torch.zeros(B, 1, K).to(self.device)

                # clip transformation matrix
                with torch.no_grad():
                    perturb_mat.data = torch.clamp(
                        perturb_mat.data, min=-self.budget, max=self.budget
                    ).detach()
                    gauss_delta.data = torch.clamp(
                        gauss_delta.data, min=self.min_sigm, max=self.max_sigm
                    ).detach()

                ker_density_mat = self.kernel_density(
                    central_points, ori_data, gauss_delta
                )  # [B, central_num, K]

                for j in range(0, self.central_num):
                    tmp_adv_data += (
                        adv_data + perturb_mat[:, j, :].unsqueeze(dim=2)
                ...
                # forward passing
                logits = self.model.forward(tmp_adv_data.permute([0, 2, 1]))[
                    "logit"
                ]  # [B, num_classes]
                ...
                if iteration % (self.num_iter // 5) == 0:
                    print(
                        "Step {}, iteration {}, success {}/{}\n"
                        "adv_loss: {:.4f}, dist_loss: {:.4f}".format(
                            binary_step,
                            iteration,
                            success_num,
                            B,
                ...
                dist_loss = torch.tensor(0.0).to(self.device)
                dist_val = self.transformation_loss(
                    tmp_adv_data, perturb_mat, gauss_delta, batch_avg=False
                )

                pred_val = pred.detach().cpu().numpy()  # [B]
                input_val = tmp_adv_data.detach().cpu().numpy()  # [B, 3, K]
                perturb_mat_val = (
                    perturb_mat.view(self.central_num, B, 3)
                    .transpose(0, 1)
                    .clone()
                    .detach()
                    .cpu()
                ...
                # update
                for e, (dist, pred, label, ii, pert, gd) in enumerate(
                    zip(
                        dist_val,
                        pred_val,
                        label_val,
                ...
                if cd_weight[0] != 0:
                    chamfer_loss = chamfer_dist(
                        tmp_adv_data, ori_data, torch.from_numpy(cd_weight)
                    )
                    dist_loss += chamfer_loss

                if ker_weight[0] != 0:
                    transformation_loss = self.transformation_loss(
                        tmp_adv_data, perturb_mat, gauss_delta
                    )
                ...
                if hide_weight[0] != 0:
                    hide_loss = (
                        self.curv_std_loss(
                            gauss_delta, central_kappa_std, self.max_sigm, self.min_sigm
                        )
                        * self.hide_weight
                    )
                    dist_loss += hide_loss.mean()

                scale_const = scale_const.float().to(self.device)
            ...
            for e, label in enumerate(label_val):
                ...
                if (
                    bestscore[e] != label
                    and bestscore[e] != -1
                    and bestdist[e] <= o_bestdist[e]
                ):
                    # success
                    lower_bound[e] = max(lower_bound[e], scale_const[e])
                    scale_const[e] = (lower_bound[e] + upper_bound[e]) / 2.0
                else:
                    # failure
    ...
    def kernel_density(self, central_points, pc, delta):
        ...
        density = torch.exp(
            -norm / (2 * delta * delta).unsqueeze(1)
        )  # [B, K, central_num]
    ...
    def transformation_loss(self, adv_data, perturb_mat, gauss_delta, batch_avg=True):
        ...
        if batch_avg:
            transformation_loss = torch.tensor(0.0).to(self.device)
            transformation_loss += torch.norm(perturb_mat)
            transformation_loss += 1 * (torch.norm(1 - gauss_delta))
        else:
            transformation_loss = torch.zeros((B)).to(self.device)
    ...
    def _get_kappa_std_ori(self, pc, normal, k=10):
        ...
        nn_kappa = (
            knn_gather(kappa_ori.unsqueeze(2), inter_KNN.idx)
            .permute(0, 3, 1, 2)[:, :, :, 1:]
            .contiguous()
        )  # [b, 1, n ,k]
    ...
    def sample_and_group_all(self, xyz, points):
        ...
        device = xyz.device
        B, N, C = xyz.shape
        new_xyz = torch.zeros(B, 1, C).to(self.device)
    ...
    def KNN_indices(self, x):
        ...
        B, _, K = pc.shape
        # pc = pc.transpose(2, 1)
        inner = -2.0 * torch.matmul(pc.transpose(2, 1), pc)  # [B, K, K]
    ...
    def farthest_point_sample(self, xyz, npoint):
        ...
        # device = xyz.device
        # B, N, C = xyz.shape
        # centroids = torch.zeros(B, npoint, dtype=torch.long).to(self.device)
        # distance = torch.ones(B, N).to(self.device) * 1e10
        # farthest = torch.randint(0, N, (B,), dtype=torch.long).to(self.device)
        # batch_indices = torch.arange(B, dtype=torch.long).to(self.device)
    ...
    def get_gradient(self, data, target):
        ...
        Args:
            ...
            target (torch.LongTensor): target output, [B]
        """
        input_data = data.clone().detach().float().to(self.device)
        input_data.requires_grad_()
        target = target.long().to(self.device)

        # forward pass
        logits = self.model.forward(input_data.permute([0, 2, 1]))["logit"]
