...
class WaveletTransformUtil(nn.Module):
    ...
    def transform(self, signal_batch: torch.Tensor) -> torch.Tensor:
        ...
        if signal_b > base_b and signal_b % base_b == 0:
            ...
            wavelets_fs = (
                self.wavelets_fs.unsqueeze(1)
                .repeat(1, step, 1, 1)
                .view(-1, *self.wavelets_fs.shape[1:])
            )
        else:
            # 如果批次大小匹配，则直接使用
            Us = self.Us
    ...
    def inverse_transform(self, coeffs_batch: torch.Tensor) -> torch.Tensor:
        ...
        if coeffs_b > base_b and coeffs_b % base_b == 0:
            ...
            Us = self.Us.unsqueeze(1).repeat(1, step, 1, 1).view(-1, *self.Us.shape[1:])
            inv_wavelets_fs = (
                self.inv_wavelets_fs.unsqueeze(1)
                .repeat(1, step, 1, 1)
                .view(-1, *self.inv_wavelets_fs.shape[1:])
            )
        else:
            Us = self.Us
    ...
    def extract_grad_wrt_coffs(
        ...
        error_per_sample = torch.linalg.norm(
            signal_batch - inv_signal_batch, dim=(1, 2)
        )  # Frobenius norm for each [N,C] matrix
        ...
        if total_norm_error > 1e-10:
            ...
            logger.warning(
                f"[total_norm_error] wavelet error is {total_norm_error:.3e}"
            )
    ...
    def get_coeffs_and_grads_advanced(
        self,
        ...
        from functools import partial  # 导入 partial 用于更清晰地创建hook
        ...
        if step > 1 and eps > 0:
            replicated_data = (
                replicated_data + torch.rand_like(replicated_data) * eps * 2 - eps
            )
        ...
        # 新增: 定义前向Hook函数，用于捕获指定层的输出
        def get_features_hook(name, module, input, output):
            """一个捕获模块输出并存入字典的Hook函数。"""
            feature_outputs[name] = output.detach()

        # 新增: 注册前向Hook
        forward_handles = []
        for name in track_modules:
            module_to_hook = attrgetter(name)(target_model)
            # 使用 functools.partial 将层名称(name)传递给hook函数
            handle = module_to_hook.register_forward_hook(
                partial(get_features_hook, name)
            )
        ...
        handles = [
            h.register_full_backward_hook(backward_hook)
            for h in [attrgetter(m)(target_model) for m in track_modules]
        ]

        coeffs_batch_replicated = self.transform(
            replicated_data.double()
        ).requires_grad_()
        inv_signal_batch_replicated = self.inverse_transform(coeffs_batch_replicated)

        # 执行前向传播，这将触发上面注册的前向Hook
        ...
        with torch.no_grad():
            for grad in captured_grads[::-1]:
                if grad.shape[1] != risk_replicated.shape[1]:
        ...
        final_risk = (
            sum_same_batch(risk_replicated, batch_index) / step
            if step > 1
            else risk_replicated
        )

        # 步骤 4: 聚合梯度本身，用于后续的智能滤波
        grad_replicated = coeffs_batch_replicated.grad
        smoothed_grad = (
            sum_same_batch(grad_replicated, batch_index) / step
            if step > 1
            else grad_replicated
        )
    ...
    def _build_bases(self, batched_pc: torch.Tensor):
        ...
        logger.info(
            f"Building wavelet bases for {batched_pc.shape[0]} graphs on device {device}..."
        )
        ...
        laplacians = sparse_normalized_laplacian(
            torch.stack(adj_matrices, dim=0)
        ).double()
        ...
        for i, adj in enumerate(adjs):
            ...
            wavelets_f = (
                torch.from_numpy(pygsp2_filter.evaluate(pygsp2_filter.G.e)).to(device).T
            )
            inv_wavelets_f = (
                torch.from_numpy(pygsp2_filter_inv.evaluate(pygsp2_filter_inv.G.e))
                .to(device)
                .T
            )
