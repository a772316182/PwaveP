...
from src.defenders.wavelet_def.local_risk import extract_feature_grad
...
def start_wavelet_gard_wrt_coff_energy(
    ...
    for batch_idx, data_item in enumerate(tqdm(loader)):
        ...
        model_risk, wavelet_coeffs, grad_wrt_wavelet_coeffs = (
            wavelet.get_coeffs_and_grads_advanced(
                target_model=model,
                signal_batch=batched_adv_pc.detach().clone(),
                label=clean_label,
                step=grad_smooth_step,
                eps=grad_smooth_eps,
            )
        )

        geo_grad = extract_feature_grad(batched_adv_pc, k=k_neighbors, mode="norm").to(
            device
        )
        risk = spectral_risk_weight * model_risk + geo_risk_weight * geo_grad
        ...
        if num_to_repair > 0:
            # 计算每个频段的全局梯度能量
            grad_energy_per_band = torch.norm(
                grad_wrt_wavelet_coeffs, p=2, dim=(0, 1, 2)
            )
            # 找到能量最高（最可疑）的k个频段的索引
            _, high_energy_bands_indices = torch.topk(
                grad_energy_per_band, k=k_bands_to_filter
            )

            logger.info(
                f"Batch {batch_idx}, filtering bands: {high_energy_bands_indices.cpu().numpy()}"
            )
            ...
            for i in range(batch_size):
                ...
                purified_coeffs[
                    i, points_to_filter, :, high_energy_bands_indices
                ] *= attenuation_factor
