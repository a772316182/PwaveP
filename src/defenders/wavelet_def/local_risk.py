...
def compute_average_distance(input_tensor, k):
    ...
    distance_matrix = torch.sqrt(
        square + square.transpose(-2, -1) - 2 * inner_product + 1e-6
    )  # B x N x N

    # find the k nearest points and compute average distance
    _, indices = torch.topk(
        distance_matrix, k, largest=False, sorted=True, dim=-1
    )  # B x N x k
...
def local_geometric_risk(avg_dist, mode="gaussian"):
    if mode == "gaussian":
        ...
        gaussian_density = (
            1 / (std * torch.sqrt(torch.tensor(2 * torch.pi)))
        ) * torch.exp(
            -0.5 * ((avg_dist - mean) / std) ** 2
        )  # B x N
...
def extract_feature_grad(points, k=20, mode="gaussian"):
