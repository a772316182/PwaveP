...
from src.defenders.dupnet.utils import *
...
class PUNet(nn.Module):
    ...
    def __init__(
            self, npoint=1024, up_ratio=2, use_normal=False, use_bn=False, use_res=False
    ):
        ...
        for k in range(len(self.npoints) - 1):
            self.FP_Modules.append(
                PointnetFPModule(mlp=[mlps[k + 1][-1], 64], bn=use_bn)
            )
        ...
        for k in range(up_ratio):
            self.FC_Modules.append(SharedMLP([in_ch, 256, 128], bn=use_bn))
    ...
    def forward(self, points, npoint=None):
        ...
        feats = (
            points[..., 3:].transpose(1, 2).contiguous() if self.use_normal else None
        )
        ...
        # [xyz, l0, l1, l2, l3]
        feats = torch.cat(
            [xyz.transpose(1, 2).contiguous(), l_feats[1], *up_feats], dim=1
        ).unsqueeze(
            -1
        )  # bs, mid_ch, N, 1
